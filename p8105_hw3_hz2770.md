...
# `{r} # instacart %>%  #   count(aisle) %>%  #   filter(n > 10000) %>%  #   ggplot(aes(y = n, color = aisle)) + #   geom_boxplot() #`
...
2.  Construct the dataset and make the “spagetti” plot. \#
    `{r} # brfss_df =  #   brfss_smart2010 %>% #   filter(response == 5) %>% #   group_by(locationabbr) %>% #   mutate(mean_data_value = mean(data_value)) %>% #   select(year, locationabbr, mean_data_value) %>% #   ggplot( #     aes(x = year, y = mean_data_value, color = locationabbr) #     ) +  #   geom_line() #    #`
...
![](p8105_hw3_hz2770_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->
